...
Asena.addCommand({on: 'text', fromMe: wk, dontAddCommandList: true, deleteCommand: false}, (async (message, match) => {
    if (message.message.startsWith('pinky') && conf.FULLEVA_NOR !== 'on') {        
...
Asena.addCommand({on: 'text', fromMe: false, deleteCommand: false}, (async (message, match) => {
        if (conf.FULLEVA_NOR == 'on' && ((!message.jid.includes('-')) || (message.jid.includes('-') && 
...
Asena.addCommand({ pattern: 'EVA ?(.*)', desc: fulleva_dsc, fromMe: true,dontAddCommandList: true, usage: '.pinky on / off' }, (async (message, match) => {
    var pinky_status = `${conf.FULLEVA_NOR}`
    if (match[1] == 'on') {
        if (pinky_status == 'on') {
